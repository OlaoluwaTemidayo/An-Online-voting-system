...
// Check if the category parameter is set in the URL and matches the "Assistant Secretary General" category
if (isset($_GET['category']) && $_GET['category'] === 'Assistant Secretary General') {
    // Retrieve the list of candidates who applied for Assistant Secretary General from the candidates table
    $sql = "SELECT * FROM candidates WHERE category = 'Assistant Secretary General'";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<input type='hidden' name='category_id' value='5'>"; // Assuming category_id 4 corresponds to 'Assistant Secretary General'
